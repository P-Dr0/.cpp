...
void ordena_alunos_nota(Aluno vetor[], int inicio, int fim){
	int meio = (inicio+fim)/2;
	...
	if(inicio < j)
		ordena_alunos_nota(vetor, inicio, j);
	if(i < fim)
		ordena_alunos_nota(vetor, i, fim);
}
...
int main(){
	...
	for(int j=0; j < (num_alunos*num_exercicios); j++){
		...
		alunos[pos].nota = alunos[pos].nota + nota;
	}
	
	for(int i=0; i< num_alunos; i++){ 
		alunos[i].nota = alunos[i].nota / num_exercicios; 
	}
	
	ordena_alunos_nota(alunos, 0, num_alunos-1);
